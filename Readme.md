...
Parameters can also be handled here via a list of [PipelineParameter](src/com/example/pojo/PipelineParameter.groovy) objects being passed in under the parameters field.

For regularly scheduled jobs triggers  are also available via the [PipelineTrigger](src/com/example/pojo/PipelineTrigger.groovy) object although the only available options here are GENERIC_WEBHOOK and CRON.
...
These pipeline objects are then handled in the [seed.groovy](pipelines/seed.groovy) file which is then called via the seed job, described by seedJob.groovy.
As a note, the ['seed job'](pipelines/seedJob.groovy) must be created & maintained manually.
...
requirements (e.g. terraform vs Java app) there is an additional subdivision of types that inherit from [CodeUnit](src/com/example/pojo/codeunit/CodeUnit.groovy) to provide
more specific configurations. Take [GoCodeUnit](src/com/example/pojo/codeunit/CodeUnit.groovy) for example. On top of the standard git repository configurations, it also
...
to ensure that all the required configs were passed in. The [JobDslService](src/com/example/services/JobDslService.groovy)
